...
public class NgsEvoucherTest {
  ...
  private NgsEvoucher ngsEvoucher;
  ...
  private void basicSetup() throws Exception {
    ...
    EVoucherView viewRestriction =
        new EVoucherView("MCjT1X5GTU", "NGS_E_VOUCHER", 10L, "1", formatter.parse(startDate),
            formatter.parse(endDate), 55L, 44L, true, "REDEEM_RESTRICTIONS", "RESTRICT_NONE", 5);
    ngsEvoucher = new NgsEvoucher(messageSource, productPaymentMethod, userPaymentMethod);
    ngsEvoucher.setEvoucher(Arrays.asList(viewRestriction));
  ...
  public void validateAtApplyEvoucher() throws Exception {
    basicSetup();
    EVoucherValidation result = ngsEvoucher.validateAtApplyEvoucher();
    assertTrue(ngsEvoucher.getRedeemRestrictionsRule().isExecuted());
    EVoucherValidation expected =
        objectMapper.readValue(
            this.getClass().getResource("/evoucher/json/evoucherValidation_NgsEvoucher_Pass.json"),
  ...
  public void validateAtPreCommitOrder() throws Exception {
    basicSetup();
    EVoucherValidation result = ngsEvoucher.validateAtPreCommitOrder();
    EVoucherValidation expected =
        objectMapper.readValue(
            this.getClass().getResource("/evoucher/json/evoucherValidation_NgsEvoucher_Pass.json"),
            EVoucherValidation.class);
  ...
  public void setEvoucherCaseRedeemRestriction() throws Exception {
    ...
    ngsEvoucher = new NgsEvoucher(messageSource, productPaymentMethod, userPaymentMethod);
    ngsEvoucher.setEvoucher(eVoucherViewList);
    EVoucherView result = ngsEvoucher.getRedeemRestrictionsRule().getEVoucherView();
